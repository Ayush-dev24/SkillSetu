// ---------------------------------------------------------------------------
// In-house career assessments: question bank + scoring + skill-level inference.
// ---------------------------------------------------------------------------
// Pure, deterministic, dependency-free module — runs in the browser AND is
// mirrored server-side (api/lib/assessments.js) so an assessment started in
// the UI can be scored on the server without shipping any scoring logic in a
// way that a client could tamper with. The server always re-derives the score
// from the submitted answers.
//
// Design:
//   - ASSESSMENTS: fixed catalog. Each assessment maps to 1-2 target skills.
//   - Questions are either multiple-choice (options) or range (1-10 slider).
//   - Scoring is a weighted sum; each option carries a points value.
//   - getLevelFromScore maps a 0-100 score to the Skill Setu 1-5 level scale.
//   - inferLevelFromAnswers is exported both for live preview (client) and
...
export interface AssessmentQuestion {
  id: string;
  text: string;
  /** 'mcq' — single choice; 'range' — numeric slider (1..10, points scale). */
  kind: 'mcq' | 'range';
...
export interface AssessmentDef {
  ...
  description: string;
  skill: string;
  /** Secondary skill also strengthened by this assessment (may be empty). */
  skill2?: string;
  durationMin: number;
...
/** Fixed catalog — the same set is mirrored in api/lib/assessments.js. */
export const ASSESSMENTS: AssessmentDef[] = [
  {
    ...
    questions: [
      {
        id: 'py-1',
        text: 'How confident are you writing Python functions with default arguments?',
        kind: 'range',
        hint: '1 = I have not tried; 10 = I write them daily without looking anything up.',
      },
      {
        id: 'py-2',
        text: 'Which of these is a valid way to iterate over both index and value in Python?',
        kind: 'mcq',
        options: [
      ...
      {
        id: 'py-3',
        text: 'When should you prefer a tuple over a list?',
        kind: 'mcq',
        options: [
      ...
      {
        id: 'py-4',
        text: 'How do you handle exceptions cleanly in Python?',
        kind: 'mcq',
        options: [
      ...
      {
        id: 'py-5',
        text: 'What does a list comprehension `[x*2 for x in range(3)]` produce?',
        kind: 'mcq',
        options: [
      ...
      {
        id: 'py-6',
        text: 'You need to load a CSV with 50,000 rows. Which tool do you reach for?',
        kind: 'mcq',
        options: [
  ...
  },
  {
    ...
    questions: [
      {
        id: 'js-1',
        text: 'How comfortable are you with arrow functions?',
        kind: 'range',
        hint: '1 = never used; 10 = I use them in every file.',
      },
      {
        id: 'js-2',
        text: 'What does `const` mean in modern JavaScript?',
        kind: 'mcq',
        options: [
      ...
      {
        id: 'js-3',
        text: 'How do you fetch data from an API in the browser?',
        kind: 'mcq',
        options: [
      ...
      {
        id: 'js-4',
        text: 'Which of these is a valid array method to transform every element?',
        kind: 'mcq',
        options: [
      ...
      {
        id: 'js-5',
        text: 'How do you safely check an object "own" property in modern JS?',
        kind: 'mcq',
        options: [
  ...
  },
  {
    ...
    questions: [
      ...
      {
        id: 'sql-2',
        text: 'How do you combine two tables on a shared key?',
        kind: 'mcq',
        options: [
      ...
      {
        id: 'sql-3',
        text: 'What does `GROUP BY department` do?',
        kind: 'mcq',
        options: [
      ...
      {
        id: 'sql-4',
        text: 'Which is the fastest way to look up one row by primary key?',
        kind: 'mcq',
        options: [
      ...
      {
        id: 'sql-5',
        text: 'How confident are you writing aggregate queries with COUNT/SUM/AVG?',
        kind: 'range',
        hint: '1 = never; 10 = I use them for daily reports.',
      },
    ],
  },
  {
    ...
    questions: [
      ...
      {
        id: 'rx-3',
        text: 'Which hook holds per-component state?',
        kind: 'mcq',
        options: [
      ...
      {
        id: 'rx-4',
        text: 'When does `useEffect(fn, [dep])` re-run?',
        kind: 'mcq',
        options: [
  ...
  },
  {
    ...
    questions: [
      {
        id: 'da-1',
        text: 'Which step comes first in most analyses?',
        kind: 'mcq',
        options: [
      ...
      {
        id: 'da-2',
        text: 'A column has blank cells. What is the safest move?',
        kind: 'mcq',
        options: [
      ...
      {
        id: 'da-3',
        text: 'Which chart best shows a trend over time?',
        kind: 'mcq',
        options: [
      ...
      {
        id: 'da-4',
        text: 'How do you summarize a group in a spreadsheet or tool?',
        kind: 'mcq',
        options: [
      ...
      {
        id: 'da-5',
        text: 'How comfortable are you with pivot tables?',
        kind: 'range',
        hint: '1 = never tried; 10 = I build them without thinking.',
      },
    ],
  },
  {
    ...
    questions: [
      {
        id: 'ay-1',
        text: 'Which is a core aim of Nadi Pariksha?',
        kind: 'mcq',
        options: [
      ...
      {
        id: 'ay-2',
        text: 'What is Panchakarma primarily?',
        kind: 'mcq',
        options: [
      ...
      {
        id: 'ay-3',
        text: 'Why is a case sheet important in clinical practice?',
        kind: 'mcq',
        options: [
      ...
      {
        id: 'ay-4',
        text: 'A patient asks about diet changes alongside therapy. What is the right approach?',
        kind: 'mcq',
        options: [
      ...
      {
        id: 'ay-5',
        text: 'How confident are you documenting a patient consultation?',
        kind: 'range',
        hint: '1 = never done it; 10 = I document every visit fully.',
      },
    ],
  },
];

/** Look up an assessment by id; undefined when unknown. */
...
export interface AnswerValue {
  /** question id */
  q: string;
  /** mcq → the selected option index; range → a 1..10 number */
  value: number | string;
}

/** Score one answer against its question. Returns points earned (0..max). */
export function scoreAnswer(question: AssessmentQuestion, raw: unknown): number {
  if (question.kind === 'range') {
    const n = Number(raw);
    const clamped = Number.isFinite(n) ? Math.max(1, Math.min(10, n)) : 1;
    // Map 1..10 → 0..3 (rounded): 1-2 → 0, 3-4 → 1, 5-7 → 2, 8-10 → 3
...
/**
 * Score a submitted assessment. The server uses this to compute the
 * authoritative result; the client uses it for an instant live preview.
 *
 * Returns:
 *   score          0-100
 *   maxPoints      total points available
 *   earnedPoints   total points earned
 *   perQuestion    per-question detail (id, earned, max)
 *   unanswered     count of questions without a value
 */
export function scoreAssessment(
  assessment: AssessmentDef,
  answers: Record<string, unknown> | AnswerValue[],
): {
  score: number;
  maxPoints: number;
  earnedPoints: number;
  perQuestion: { id: string; earned: number; max: number }[];
  unanswered: number;
} {
  const map: Record<string, unknown> = Array.isArray(answers)
  ...
  const perQuestion = assessment.questions.map((q) => {
    const max = q.kind === 'range' ? 3 : q.options?.reduce((s, o) => Math.max(s, o.points), 0) ?? 0;
    maxPoints += max;
...
/**
 * Map a 0-100 assessment score to the Skill Setu 1-5 level scale
 * (used for skill level inference + resumes + profile).
 */
...
/** Did the assessment pass the built-in pass threshold? */
export function passedAssessment(assessment: AssessmentDef, score: number): boolean {
...
export interface AssessmentOutcome {
  ...
  /** Human-friendly result summary for the profile & dashboard. */
  summary: string;
}

/**
 * Full outcome for a completed assessment: score → level → which skills are
 * strengthened. The server persists this outcome; the browser renders it.
 */
export function outcomeForAssessment(
  assessment: AssessmentDef,
  answers: Record<string, unknown> | AnswerValue[],
): AssessmentOutcome {
